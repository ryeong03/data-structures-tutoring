...
export type State={workspace?:Workspace;workspaces?:Workspace[];isAdmin?:boolean;me:(Member|PublicMember)&{email?:string};members:(Member|PublicMember)[];weeks:Week[];notices:Notice[];materials:Material[];questions:Question[];reports:Report[];quizzes:Quiz[];attempts:Attempt[];rsvps:Rsvp[];config:{zoomUrl:string};tutorWeeks?:TutorWeek[];aiUsage?:{at:string;weekId:number;model:string;usage:{input_tokens?:number;output_tokens?:number}}[];activityLog?:Activity[];inviteActive?:boolean};
